...
BlocklyDuino.init = function () {
	...
	BlocklyDuino.workspace.addChangeListener(function (event) {
		// Read the current language (default to 'arduino')
		const prog = window.localStorage.prog;
		// Only trigger preview if not in Arduino mode
		if (prog !== 'arduino') {
			window.localStorage.prog = '';
...
BlocklyDuino.renderArduinoCodePreview = function () {
	...
	switch (prog) {
		...
		default: // 'arduino' (C++)
			$('#lang_toggle .btn').removeClass('active');
			$('#lang_toggle input[value="arduino"]')
				.prop('checked', true)
				.closest('label').addClass('active');
			$('#pre_previewArduino').text(Blockly.Arduino.workspaceToCode(BlocklyDuino.workspace));
			$('#pre_previewArduino').html(prettyPrintOne($('#pre_previewArduino').html(), 'cpp'));
			window.localStorage.prog = 'arduino';
			break;
	}
};
...
BlocklyDuino.bindFunctions = function () {
	...
	$('#btn_preview').on("click", function () {
		...
		$('#lang_toggle input[value="arduino"]')
			.prop('checked', true)
			.closest('label').addClass('active');
		window.localStorage.prog = '';     // persist choice
		BlocklyDuino.renderArduinoCodePreview();
		$("#toggle").toggle("slide")
	});
	$('#lang_toggle input[name="lang"]').change(function () {
		var lang = $(this).val();            // "arduino", "python", or "java"
		window.localStorage.prog = lang;     // persist choice
		BlocklyDuino.renderArduinoCodePreview();
	});
